...
test("readBridgeConfig keeps safe defaults and explicit overrides", () => {
  ...
  const persistedKeepAwakeConfig = readBridgeConfig({
    env: {
      KODER_DEVICE_STATE_DIR: "/tmp/koder-state",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/koder-package",
    fsImpl: {
      existsSync(targetPath) {
        return targetPath === "/tmp/koder-state/daemon-config.json";
      },
      readFileSync(targetPath) {
        if (targetPath === "/tmp/koder-state/daemon-config.json") {
  ...
  const macEndpointConfig = readBridgeConfig({
    env: { KODER_CODEX_ENDPOINT: "ws://localhost:8080" },
    platform: "darwin",
    runtimeRoot: "/tmp/koder-package",
    fsImpl: {
      existsSync: () => false,
  ...
  const linuxCommandConfig = readBridgeConfig({
    env: { KODER_REFRESH_COMMAND: "echo refresh" },
    platform: "linux",
    runtimeRoot: "/tmp/koder-package",
    fsImpl: {
      existsSync: () => false,
  ...
  const explicitOnConfig = readBridgeConfig({
    env: {
      KODER_CODEX_ENDPOINT: "ws://localhost:8080",
      KODER_REFRESH_ENABLED: "true",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/koder-package",
    fsImpl: {
  ...
  const explicitOffConfig = readBridgeConfig({
    env: {
      KODER_REFRESH_COMMAND: "echo refresh",
      KODER_REFRESH_ENABLED: "false",
      KODER_KEEP_MAC_AWAKE: "false",
    },
    platform: "darwin",
    runtimeRoot: "/tmp/koder-package",
    fsImpl: {
...
test("readBridgeConfig uses only the packaged relay default outside a source checkout", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "koder-package-"));
...
test("readBridgeConfig preserves reverse-proxy subpaths when deriving push URLs", () => {
  const config = readBridgeConfig({
    env: {
      KODER_PUSH_SERVICE_URL: "https://relay.example/koder",
    },
    runtimeRoot: "/workspace/bridge",
    fsImpl: {
      existsSync() {
  ...
  assert.equal(config.pushServiceUrl, "https://relay.example/koder");
});
...
test("readBridgeConfig disables managed push defaults when a self-hosted relay override is set", () => {
  ...
  fs.writeFileSync(
    path.join(srcDir, "private-defaults.json"),
    JSON.stringify({ relayUrl: "wss://relay.example/koder/relay" }),
    "utf8"
  );

  const config = readBridgeConfig({
    env: {
      KODER_RELAY: "wss://self-host.example/relay",
    },
    runtimeRoot: tempRoot,
    fsImpl: fs,
  });

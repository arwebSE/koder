...
// Depends on: node:test, node:assert/strict, child_process, path, ../package.json, ../bin/koder
...
const { main } = require("../bin/koder");

test("koder --version prints the package version", () => {
  const cliPath = path.join(__dirname, "..", "bin", "koder.js");
...
test("koder restart reuses the macOS service start flow", async () => {
  const calls = [];
  const messages = [];

  await main({
    argv: ["node", "koder", "restart"],
    platform: "darwin",
    consoleImpl: {
      log(message) {
  ...
  assert.deepEqual(messages, [
    "[koder] macOS bridge service restarted.",
  ]);
});

test("koder status --json exposes daemon metadata for companion apps", async () => {
  const writes = [];

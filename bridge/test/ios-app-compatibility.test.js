...
test("buildIOSAppCompatibilitySnapshot blocks iPhone 1.0 on bridge 1.3.8", () => {
  ...
  assert.equal(snapshot.downgradeCommand, "npm install -g koder@1.3.7");
  assert.match(snapshot.message, /requires Koder iPhone 1\.1 or later/i);
  assert.match(snapshot.message, /install Koder bridge 1\.3\.7 to keep using iPhone 1\.0/i);
});
...
test("buildCachedIOSAppCompatibilityWarning warns when the last seen iPhone app is 1.0", () => {
  ...
  assert.match(warning, /requires Koder iPhone 1\.1 or later/i);
  assert.match(warning, /Update the iPhone app from the App Store first/i);
  assert.match(warning, /npm install -g koder@1\.3\.7/i);
});

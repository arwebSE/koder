...
function makeTempRepo() {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "koder-git-handler-"));
  git(repoDir, "init", "-b", "main");
  git(repoDir, "config", "user.name", "Koder Tests");
  git(repoDir, "config", "user.email", "tests@example.com");
  fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n");
  fs.mkdirSync(path.join(repoDir, "bridge", "src"), { recursive: true });
  fs.writeFileSync(path.join(repoDir, "bridge", "src", "index.js"), "export const ready = true;\n");
  git(repoDir, "add", "README.md");
  git(repoDir, "add", "bridge/src/index.js");
...
function makeBareRemote() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "koder-git-handler-remote-"));
}
...
test("gitBranches scopes worktree paths to the current project subdirectory", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "bridge");
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-feature`);
  ...
  try {
    ...
    assert.equal(
      ...
      canonicalPath(path.join(siblingWorktree, "bridge"))
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(siblingWorktree, { recursive: true, force: true });
...
test("gitBranches scopes local checkout path for subdirectory worktrees", async () => {
  const repoDir = makeTempRepo();
  const localProjectDir = path.join(repoDir, "bridge");
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-feature`);
  const siblingProjectDir = path.join(siblingWorktree, "bridge");
...
test("gitCheckout surfaces a specific error when the requested branch does not exist locally", async () => {
  ...
  try {
    await assert.rejects(
      __test.gitCheckout(repoDir, { branch: "koder/missing" }),
      (error) =>
        error?.errorCode === "branch_not_found"
          && error?.userMessage === "Branch 'koder/missing' does not exist locally."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch normalizes bare names into koder/* and checks out the new branch", async () => {
  const repoDir = makeTempRepo();

  try {
    const result = await __test.gitCreateBranch(repoDir, { name: "new-branch" });

    assert.equal(result.branch, "koder/new-branch");
    assert.equal(result.status?.branch, "koder/new-branch");
    assert.equal(git(repoDir, "rev-parse", "--abbrev-ref", "HEAD"), "koder/new-branch");
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
});

test("normalizeCreatedBranchName avoids double-prefixing koder branches", () => {
  assert.equal(__test.normalizeCreatedBranchName("feature/foo"), "koder/feature/foo");
  assert.equal(__test.normalizeCreatedBranchName("koder/feature/foo"), "koder/feature/foo");
  assert.equal(__test.normalizeCreatedBranchName("my new branch"), "koder/my-new-branch");
  assert.equal(__test.normalizeCreatedBranchName("feature / login page"), "koder/feature/login-page");
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "invalid_branch_name"
          && error?.userMessage === "Branch 'koder/feature..oops' is not a valid Git branch name."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch rejects duplicate branch names with a specific error", async () => {
  ...
  try {
    git(repoDir, "branch", "koder/already-there");

    await assert.rejects(
      __test.gitCreateBranch(repoDir, { name: "already-there" }),
      (error) =>
        error?.errorCode === "branch_exists"
          && error?.userMessage === "Branch 'koder/already-there' already exists."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitBranches hides remote-only branches from the local selector list", async () => {
  ...
  try {
    pushRemoteOnlyBranch(repoDir, remoteDir, "koder/remote-only");

    const result = await __test.gitBranches(repoDir);

    assert.ok(!result.branches.includes("koder/remote-only"));
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
  }
...
test("gitBranches preserves the repo default branch even when it is not checked out locally", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "koder/topic");
    ...
    assert.ok(result.branches.includes("koder/topic"));
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch rejects names that already exist only on origin", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "branch_exists"
          && error?.userMessage === "Branch 'koder/remote-only' already exists on origin. Check it out locally instead of creating a new branch."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
...
test("gitStatus marks a branch as published when origin has it even without local upstream", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "koder/published-no-upstream");
...
test("gitCreateWorktree creates a managed worktree under CODEX_HOME/worktrees", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "bridge");
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "koder-codex-home-"));
  ...
  try {
    ...
    assert.equal(result.branch, "koder/new-worktree");
    assert.equal(result.alreadyExisted, false);
    assert.ok(result.worktreePath.startsWith(managedWorktreesRoot));
    assert.equal(path.basename(result.worktreePath), "bridge");
    assert.equal(git(result.worktreePath, "rev-parse", "--abbrev-ref", "HEAD"), "koder/new-worktree");
...
test("gitCreateWorktree reuses an existing worktree for the same koder branch", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "bridge");
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "koder-codex-home-"));
  const previousCodexHome = process.env.CODEX_HOME;
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-koder-existing`);

  process.env.CODEX_HOME = codexHome;

  try {
    git(repoDir, "branch", "koder/existing");
    git(repoDir, "worktree", "add", siblingWorktree, "koder/existing");
    ...
    assert.equal(result.branch, "koder/existing");
    assert.equal(result.alreadyExisted, true);
    assert.equal(result.worktreePath, canonicalPath(path.join(siblingWorktree, "bridge")));
  } finally {
    if (previousCodexHome === undefined) {
      delete process.env.CODEX_HOME;
    } else {
...
test("gitCreateWorktree rejects a reused local branch name before ignoring the chosen base branch", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "branch_exists"
          && error?.userMessage === "Branch 'koder/already-there' already exists locally. Choose another name or open that branch instead."
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateWorktree carries tracked and untracked changes into the new worktree and cleans local", async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoDir, "bridge", "scratch.txt"), "carry me\n");
    ...
    assert.equal(git(repoDir, "status", "--short"), "");
    assert.equal(fs.existsSync(path.join(repoDir, "bridge", "scratch.txt")), false);
...
test("gitCreateWorktree can copy tracked and untracked changes into the new worktree without cleaning local", async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoDir, "bridge", "scratch.txt"), "keep me too\n");
    ...
    assert.match(git(repoDir, "status", "--short"), /bridge\/src\/index\.js/);
    assert.equal(
      fs.readFileSync(path.join(repoDir, "bridge", "scratch.txt"), "utf8"),
      "keep me too\n"
    );

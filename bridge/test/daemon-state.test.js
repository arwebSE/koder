...
test("daemon-state stores config, pairing payloads, and status under the koder state dir", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    assert.equal(resolveKoderStateDir(), rootDir);
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "koder-daemon-state-"));

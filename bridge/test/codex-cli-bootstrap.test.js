...
test("ensureCodexCLI installs Codex when it is missing", () => {
  ...
  assert.deepEqual(messages, [
    "[koder] Checking Codex CLI...",
    "[koder] Codex CLI not found.",
    "[koder] Installing Codex CLI via npm (@openai/codex@latest)...",
    "[koder] Codex CLI installed (0.120.0).",
  ]);
});
...
test("ensureCodexCLI updates Codex when it is already installed", () => {
  ...
  assert.deepEqual(messages, [
    "[koder] Checking Codex CLI...",
    "[koder] Codex CLI found (0.118.0).",
    "[koder] Updating Codex CLI via npm (@openai/codex@latest)...",
    "[koder] Codex CLI updated (0.120.0).",
  ]);
});
...
test("shouldSkipCodexBootstrap respects the opt-out env flag", () => {
  assert.equal(shouldSkipCodexBootstrap({ KODER_SKIP_CODEX_BOOTSTRAP: "1" }), true);
  assert.equal(shouldSkipCodexBootstrap({ KODER_SKIP_CODEX_BOOTSTRAP: "true" }), true);
  assert.equal(shouldSkipCodexBootstrap({ KODER_SKIP_CODEX_BOOTSTRAP: "0" }), false);
});

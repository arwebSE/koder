...
test("buildLaunchAgentPlist points launchd at run-service with koder state paths", () => {
  const plist = buildLaunchAgentPlist({
    homeDir: "/Users/tester",
    pathEnv: "/usr/local/bin:/usr/bin",
    stateDir: "/Users/tester/.koder",
    stdoutLogPath: "/Users/tester/.koder/logs/bridge.stdout.log",
    stderrLogPath: "/Users/tester/.koder/logs/bridge.stderr.log",
    nodePath: "/usr/local/bin/node",
    cliPath: "/tmp/koder/bin/koder.js",
  });

  assert.match(plist, /<string>com\.koder\.bridge<\/string>/);
  ...
  assert.match(plist, /<key>KODER_DEVICE_STATE_DIR<\/key>/);
});
...
test("resolveLaunchAgentPlistPath writes into the user's LaunchAgents folder", () => {
  assert.equal(
    ...
    path.join("/Users/tester", "Library", "LaunchAgents", "com.koder.bridge.plist")
  );
});
    ...
    assert.deepEqual(calls, [
      [
        ...
        [
          "bootout",
          `gui/${process.getuid()}`,
          path.join(process.env.HOME, "Library", "LaunchAgents", "com.koder.bridge.plist"),
        ],
      ],
      [
        "launchctl",
        [
          "bootout",
          `gui/${process.getuid()}/com.koder.bridge`,
        ],
      ],
    ]);
  });
...
test("getMacOSBridgeServiceStatus reports launchd + runtime metadata together", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const plistPath = path.join(rootDir, "LaunchAgents", "com.koder.bridge.plist");
    fs.mkdirSync(path.dirname(plistPath), { recursive: true });
    fs.writeFileSync(plistPath, "plist");

    const status = getMacOSBridgeServiceStatus({
      platform: "darwin",
      env: { HOME: rootDir, KODER_DEVICE_STATE_DIR: rootDir },
      execFileSyncImpl() {
        return "pid = 55";
      },
    });
...
function withTempDaemonEnv(run) {
  const previousDir = process.env.KODER_DEVICE_STATE_DIR;
  const previousHome = process.env.HOME;
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "koder-launch-agent-"));
  process.env.KODER_DEVICE_STATE_DIR = rootDir;
  ...
  try {
    ...
    if (previousDir === undefined) {
      delete process.env.KODER_DEVICE_STATE_DIR;
    } else {
      process.env.KODER_DEVICE_STATE_DIR = previousDir;
    }

...
test("secure transport round-trips encrypted payloads after a trusted reconnect handshake", () => {
  ...
  const infoPrefix = `koder-e2ee-v1|session-2|mac-2|phone-2|${serverHello.keyEpoch}`;
...
test("bootstrap allows a fresh pairing to replace the trusted iPhone", () => {
  const macIdentity = createOkpKeyPair("ed25519");
  ...
  finishHandshake({
    ...
    handshakeMode: HANDSHAKE_MODE_BOOTSTRAP,
    lastAppliedBridgeOutboundSeq: 0,
  });
...
test("legacy bootstrap handshake mode is accepted as bootstrap", () => {
  const macIdentity = createOkpKeyPair("ed25519");
  const phoneIdentity = createOkpKeyPair("ed25519");
  const phoneEphemeral = createOkpKeyPair("x25519");
  const secureTransport = createBridgeSecureTransport({
  ...
  secureTransport.handleIncomingWireMessage(
    JSON.stringify({
      ...
      protocolVersion: 1,
      sessionId: "session-legacy",
      handshakeMode: HANDSHAKE_MODE_BOOTSTRAP_LEGACY,
  ...
  const serverHello = controlMessages.find((message) => message.kind === "serverHello");
  assert.ok(serverHello, "expected serverHello");
  assert.equal(serverHello.handshakeMode, HANDSHAKE_MODE_BOOTSTRAP);
});

test("pairing alias handshake mode is accepted as bootstrap", () => {
  const macIdentity = createOkpKeyPair("ed25519");
  const phoneIdentity = createOkpKeyPair("ed25519");
  const phoneEphemeral = createOkpKeyPair("x25519");
  const secureTransport = createBridgeSecureTransport({
...
test("bootstrap starts a fresh replay window instead of leaking buffered messages", () => {
  const macIdentity = createOkpKeyPair("ed25519");
  const phoneIdentity = createOkpKeyPair("ed25519");
...
test("rebinding the relay socket replays bridge output from the last phone ack", () => {
  ...
  const salt = createHash("sha256").update(transcriptBytes).digest();
  const infoPrefix = `koder-e2ee-v1|session-5|mac-5|phone-5|${serverHello.keyEpoch}`;
  const macToPhoneKey = Buffer.from(
    hkdfSync("sha256", sharedSecret, salt, Buffer.from(`${infoPrefix}|macToPhone`, "utf8"), 32)
  );
...
test("resume replay does not advance the replay watermark before a phone ack", () => {
  ...
  const salt = createHash("sha256").update(transcriptBytes).digest();
  const infoPrefix = `koder-e2ee-v1|session-6|mac-6|phone-6|${serverHello.keyEpoch}`;
  const macToPhoneKey = Buffer.from(
    hkdfSync("sha256", sharedSecret, salt, Buffer.from(`${infoPrefix}|macToPhone`, "utf8"), 32)
  );
...
function buildTranscriptBytes({
  ...
  return Buffer.concat([
    encodeLengthPrefixedUTF8("koder-e2ee-v1"),

...
// Layer: CLI helper
// Exports: createBridgeSecureTransport, SECURE_PROTOCOL_VERSION, PAIRING_PAYLOAD_VERSION
...
const HANDSHAKE_TAG = "koder-e2ee-v1";
const HANDSHAKE_MODE_BOOTSTRAP = "bootstrap";
const HANDSHAKE_MODE_BOOTSTRAP_LEGACY = "qr_bootstrap";
const HANDSHAKE_MODE_BOOTSTRAP_LEGACY_ALIASES = new Set([
  HANDSHAKE_MODE_BOOTSTRAP_LEGACY,
  "pairing",
  "pairing_bootstrap",
  "self_host_bootstrap",
  "self-host-bootstrap",
...
function createBridgeSecureTransport({
  ...
  function handleClientHello(message, sendControlMessage) {
    ...
    const rawHandshakeMode = normalizeNonEmptyString(message.handshakeMode);
    ...
    if (handshakeMode !== HANDSHAKE_MODE_BOOTSTRAP && handshakeMode !== HANDSHAKE_MODE_TRUSTED_RECONNECT) {
      console.warn(
        `[koder][secure] rejected unknown handshake mode=${rawHandshakeMode || "missing"} `
        + `session=${shortId(sessionId)} mac=${shortId(currentDeviceState.macDeviceId)}`
      );
      sendControlMessage(createSecureError({
        code: "invalid_handshake_mode",
        message: `The client requested an unknown secure pairing mode: ${rawHandshakeMode || "missing"}.`,
      }));
      return;
    }

    if (handshakeMode === HANDSHAKE_MODE_BOOTSTRAP && Date.now() > currentPairingExpiresAt) {
      sendControlMessage(createSecureError({
    ...
    const expiresAtForTranscript = handshakeMode === HANDSHAKE_MODE_BOOTSTRAP
  ...
  function handleClientAuth(message, sendControlMessage) {
    ...
    if (
      pendingHandshake.handshakeMode === HANDSHAKE_MODE_BOOTSTRAP
    ...
    if (pendingHandshake.handshakeMode === HANDSHAKE_MODE_BOOTSTRAP) {
  ...
  // Starts each fresh bootstrap with a clean catch-up window for the single trusted phone.
...
function debugSecureLog(message) {
  console.log(`[koder][secure] ${message}`);
}

function normalizeHandshakeMode(value) {
  const normalized = normalizeNonEmptyString(value);
  if (HANDSHAKE_MODE_BOOTSTRAP_LEGACY_ALIASES.has(normalized)) {
    return HANDSHAKE_MODE_BOOTSTRAP;
  }
  return normalized;
}

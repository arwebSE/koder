...
async function stashChangesForWorktreeHandoff(cwd, pathspecArgs = []) {
  const stashLabel = `koder-worktree-handoff-${randomBytes(6).toString("hex")}`;
  const output = await git(
    cwd,
    "stash",
    "push",
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(os.tmpdir(), `koder-worktree-copy-${randomBytes(6).toString("hex")}.patch`);
...
function normalizeCreatedBranchName(rawName) {
  ...
  if (normalized.startsWith("koder/")) {
    return normalized;
  }
  return `koder/${normalized}`;
}

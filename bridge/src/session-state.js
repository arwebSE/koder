...
// Purpose: Persists the latest active Koder thread so the user can reopen it on the Mac for handoff.
// Layer: CLI helper
...
const STATE_DIR = path.join(os.homedir(), ".koder");
...
function openLastActiveThread({ bundleId = DEFAULT_BUNDLE_ID } = {}) {
  ...
  if (!threadId) {
    throw new Error("No remembered Koder thread found yet.");
  }

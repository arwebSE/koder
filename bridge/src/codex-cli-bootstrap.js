...
// Purpose: Detects, installs, and updates the global Codex CLI with transparent logs for Koder installs and startup flows.
// Layer: CLI helper
...
const SKIP_BOOTSTRAP_ENV_NAME = "KODER_SKIP_CODEX_BOOTSTRAP";
...
function ensureCodexCLI({
  ...
  if (shouldSkipCodexBootstrap(env)) {
    logInfo(logger, `[koder] Skipping Codex CLI bootstrap because ${SKIP_BOOTSTRAP_ENV_NAME}=1.`);
    return {
      status: "skipped",
      versionBefore: null,
      versionAfter: null,
  ...
  logInfo(logger, "[koder] Checking Codex CLI...");
  ...
  if (versionBefore) {
    logInfo(logger, `[koder] Codex CLI found (${versionBefore}).`);
  } else {
    logInfo(logger, "[koder] Codex CLI not found.");
  }
  ...
  if (!npmVersion) {
    logWarn(
      logger,
      "[koder] npm is unavailable, so Koder could not install or update the Codex CLI automatically."
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(logger, `[koder] ${actionVerb} Codex CLI via npm (${CODEX_PACKAGE_SPEC})...`);
  ...
  try {
    ...
    logWarn(
      logger,
      `[koder] Codex CLI ${versionBefore ? "update" : "install"} failed. ${message}`
    );
    return {
      status: "failed",
      versionBefore,
  ...
  if (!versionAfter) {
    logWarn(
      logger,
      "[koder] Codex CLI bootstrap finished, but `codex --version` is still unavailable in this shell."
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(
    logger,
    `[koder] Codex CLI ${versionBefore ? "updated" : "installed"} (${versionAfter}).`
  );
  return {

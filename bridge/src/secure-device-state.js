...
const DEFAULT_STORE_DIR = path.join(os.homedir(), ".koder");
const DEFAULT_STORE_FILE = path.join(DEFAULT_STORE_DIR, "device-state.json");
const KEYCHAIN_SERVICE = "com.koder.bridge.device-state";
...
function loadOrCreateBridgeDeviceState() {
  ...
  if (fileRecord.error) {
    if (keychainRecord.state) {
      warnOnce(
        "[koder] Recovering the canonical device-state.json from the legacy Keychain pairing mirror."
      );
...
// Removes the saved bridge identity/trust state so the next `koder up` requires a fresh client bootstrap.
...
function resolveStoreDir() {
  return normalizeNonEmptyString(process.env.KODER_DEVICE_STATE_DIR) || DEFAULT_STORE_DIR;
}

function resolveStoreFile() {
  return normalizeNonEmptyString(process.env.KODER_DEVICE_STATE_FILE)
    || path.join(resolveStoreDir(), "device-state.json");
}

function resolveKeychainMirrorFile() {
  return normalizeNonEmptyString(process.env.KODER_DEVICE_STATE_KEYCHAIN_MOCK_FILE);
}
...
function reconcileLegacyKeychainMirror(canonicalState, keychainRecord) {
  if (keychainRecord.error) {
    warnOnce("[koder] Ignoring unreadable legacy Keychain pairing mirror; using canonical device-state.json.");
    return;
  }
  ...
  warnOnce("[koder] Canonical bridge pairing state differs from the legacy Keychain mirror; using device-state.json.");
  writeKeychainStateString(JSON.stringify(canonicalState, null, 2));
}
...
function corruptedStateError(source, error) {
  ...
  return new Error(
    `The saved Koder pairing state in ${source} is unreadable. `
      + "Run `koder reset-pairing` to start fresh."
      + (detail ? ` (${detail})` : "")
  );
}

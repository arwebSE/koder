...
function watchThreadRollout(threadId = "") {
  ...
  console.log(`[koder] Watching thread ${resolvedThreadId}`);
  console.log(`[koder] Rollout file: ${rolloutPath}`);
  console.log("[koder] Waiting for new persisted events... (Ctrl+C to stop)");
...
function resolveThreadId(threadId) {
  ...
  throw new Error("No thread id provided and no remembered Koder thread found.");
}

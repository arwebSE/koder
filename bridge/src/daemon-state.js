...
const DEFAULT_STATE_DIR_NAME = ".koder";
...
// Reuses the existing Koder state root so daemon mode keeps the same local-first storage model.
function resolveKoderStateDir({ env = process.env, osImpl = os } = {}) {
  return normalizeNonEmptyString(env.KODER_DEVICE_STATE_DIR)
    || path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME);
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolveKoderStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolveKoderStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolveKoderStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolveKoderStateDir(options), LOGS_DIR);
}
...
// Captures the last known service heartbeat so `koder status` does not depend on launchctl output alone.
...
function ensureKoderStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolveKoderStateDir(options), { recursive: true });
}

function ensureKoderLogsDir({ fsImpl = fs, ...options } = {}) {

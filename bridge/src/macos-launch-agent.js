...
// Purpose: Owns macOS-only launchd install/start/stop/status helpers for the background Koder bridge.
// Layer: CLI helper
// Exports: start/stop/status helpers plus the launchd service runner used by `koder up`.
// Depends on: child_process, fs, os, path, ./bridge, ./daemon-state, ./codex-desktop-refresher, ./pairing, ./secure-device-state
...
const { printPairingSessionSummary } = require("./pairing");
...
const {
  ...
  resolveKoderStateDir,
  writeBridgeStatus,
  writeDaemonConfig,
  writePairingSession,
} = require("./daemon-state");

const SERVICE_LABEL = "com.koder.bridge";
const DEFAULT_PAIRING_WAIT_TIMEOUT_MS = 10_000;
const DEFAULT_PAIRING_WAIT_INTERVAL_MS = 200;

// Runs the bridge inside launchd while keeping pairing details in the foreground CLI command.
function runMacOSBridgeService({ env = process.env } = {}) {
  ...
  if (!config?.relayUrl) {
    ...
    // Clear any stale pairing payload so the CLI does not keep showing dead bootstrap state.
    ...
    console.error(`[koder] ${message}`);
    return;
  }

  startBridge({
    config,
    printPairingDetails: false,
...
async function startMacOSBridgeService({
  ...
  cliPath = path.resolve(__dirname, "..", "bin", "koder.js"),
  ...
  ensureKoderStateDir({ env, fsImpl, osImpl });
  ensureKoderLogsDir({ env, fsImpl, osImpl });
...
function printMacOSBridgeServiceStatus(options = {}) {
  ...
  console.log(`[koder] Service label: ${status.label}`);
  console.log(`[koder] Installed: ${status.installed ? "yes" : "no"}`);
  console.log(`[koder] Launchd loaded: ${status.launchdLoaded ? "yes" : "no"}`);
  console.log(`[koder] PID: ${status.launchdPid || status.bridgeStatus?.pid || "unknown"}`);
  console.log(`[koder] Bridge state: ${bridgeState}`);
  console.log(`[koder] Connection: ${connectionStatus}`);
  console.log(`[koder] Pairing payload: ${pairingCreatedAt}`);
  console.log(`[koder] Stdout log: ${status.stdoutLogPath}`);
  console.log(`[koder] Stderr log: ${status.stderrLogPath}`);
}

function printMacOSBridgePairingInfo({ pairingSession = null, env = process.env, fsImpl = fs } = {}) {
  ...
  printPairingSessionSummary(nextPairingSession);
}
...
function writeLaunchAgentPlist({
  ...
  const stateDir = resolveKoderStateDir({ env, osImpl });
...
<dict>
  ...
  <dict>
    ...
    <key>KODER_DEVICE_STATE_DIR</key>
...
async function waitForFreshPairingSession({
  ...
  throw new Error(
    `Timed out waiting for the macOS bridge service to publish pairing details. `
...
function assertRelayConfigured(config) {
  ...
  throw new Error("No relay URL configured. Run ./run-local-koder.sh or set KODER_RELAY before enabling the macOS bridge service.");
}

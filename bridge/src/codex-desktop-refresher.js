...
class CodexDesktopRefresher {
  ...
  // Keeps one lightweight rollout watcher alive for the current Koder-controlled thread.
...
function readBridgeConfig({
  ...
  const advertisedRelayUrl = readFirstDefinedEnv(
    ["KODER_ADVERTISED_RELAY"],
    relayUrl,
    env
  );
  ...
  const codexEndpoint = readFirstDefinedEnv(
    ["KODER_CODEX_ENDPOINT"],
    "",
    env
  );
  const refreshCommand = readFirstDefinedEnv(
    ["KODER_REFRESH_COMMAND", "KODER_ON_PHONE_MESSAGE"],
    "",
    env
  );
  const explicitRefreshEnabled = readOptionalBooleanEnv(["KODER_REFRESH_ENABLED"], env);
  const explicitKeepMacAwakeEnabled = readOptionalBooleanEnv(["KODER_KEEP_MAC_AWAKE"], env);
  ...
  return {
    ...
    pushServiceUrl: readFirstDefinedEnv(
      ["KODER_PUSH_SERVICE_URL"],
      defaultPushServiceUrl,
      env
    ),
    pushPreviewMaxChars: parseIntegerEnv(
      readFirstDefinedEnv(["KODER_PUSH_PREVIEW_MAX_CHARS"], "160", env),
      160
    ),
    ...
    refreshDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(["KODER_REFRESH_DEBOUNCE_MS"], String(DEFAULT_DEBOUNCE_MS), env),
      DEFAULT_DEBOUNCE_MS
    ),
    ...
    codexEndpoint,
    refreshCommand,
    codexBundleId: readFirstDefinedEnv(["KODER_CODEX_BUNDLE_ID"], DEFAULT_BUNDLE_ID, env),
...
function isSourceCheckout(runtimeRoot, fsImpl) {
  ...
  return path.basename(runtimeRoot) === "bridge"

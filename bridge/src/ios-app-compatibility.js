...
const LEGACY_BRIDGE_DOWNGRADE_COMMAND = `npm install -g koder@${LEGACY_BRIDGE_VERSION_FOR_IOS_1_0}`;
...
function buildLegacyIOSAppCompatibilityMessage({
  ...
  return `Koder bridge ${normalizedBridgeVersion} requires Koder iPhone `
    + `${MINIMUM_SUPPORTED_IOS_APP_VERSION} or later. `
    + `Update the iPhone app from the App Store first, or install Koder bridge `
...
function buildCachedIOSAppCompatibilityWarning({
  ...
  return formatNoticeBox({
    title: "!!! WARNING !!!",
    lines: [
      `Koder bridge ${snapshot.bridgeVersion || "latest"} requires Koder iPhone ${snapshot.minimumSupportedIOSAppVersion} or later.`,

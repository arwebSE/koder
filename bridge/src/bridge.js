...
// Layer: CLI service
// Exports: startBridge
// Depends on: ws, crypto, os, ./pairing, ./codex-desktop-refresher, ./codex-transport, ./rollout-watch, ./voice-handler, ./ios-app-compatibility
...
const RELAY_HISTORY_IMAGE_REFERENCE_URL = "koder://history-image-elided";
const RELAY_THREAD_READ_TURN_LIMIT = 12;
const RELAY_THREAD_RESUME_TURN_LIMIT = 4;
function startBridge({
  config: explicitConfig = null,
  printPairingDetails = true,
  ...
  if (!relayBaseUrl) {
    console.error("[koder] No relay URL configured.");
    console.error("[koder] In a source checkout, run ./run-local-koder.sh or set KODER_RELAY.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[koder] ${(error && error.message) || "Failed to load the saved bridge pairing state."}`);
    process.exit(1);
  }
  ...
  codex.onError((error) => {
    ...
    if (config.codexEndpoint) {
      console.error(`[koder] Failed to connect to Codex endpoint: ${config.codexEndpoint}`);
    } else {
      console.error("[koder] Failed to start `codex app-server`.");
      console.error(`[koder] Launch command: ${codex.describe()}`);
      console.error("[koder] Make sure the Codex CLI is installed and that the launcher works on this OS.");
    }
  ...
  function startRelayWatchdog(trackedSocket) {
    ...
    relayWatchdogTimer = setInterval(() => {
      ...
      if (hasRelayConnectionGoneStale(lastRelayActivityAt)) {
        console.warn("[koder] relay heartbeat stalled; forcing reconnect");
  ...
  const pairingSession = { pairingPayload };
  onPairingSession?.(pairingSession);
  if (printPairingDetails) {
    printPairingSessionSummary(pairingSession);
  }
  ...
  // Refreshes the relay's trusted-mac index after bootstrap locks in a phone identity.
  ...
  function start() {
    ...
    try {
      ...
      nextChild.on?.("error", (error) => {
        consoleImpl.warn(`[koder] Failed to hold the Mac awake while the bridge is active: ${error.message}`);
      });
      nextChild.on?.("exit", () => {
        if (child === nextChild) {
          child = null;
      ...
      consoleImpl.warn(
        `[koder] Failed to start the bridge wake assertion: ${(error && error.message) || "unknown error"}`
      );
      child = null;
    }
  }
